...
export const ComplianceDetail = (props) => {
    ...
    const sendMessage = () => {
        ...
        fetch(getBaseUrl() + "message/forwardMessage", {
            method: "POST",
            headers: {
    ...
    const removeFile = (i) => {
        ...
        setRelatedDocumentLink([...relatedDocument.filter((_, index) => index !== i)]);
    };
    const change = (e) => {
        setMember(e);
    }
    ...
    const updatePage = () => {
        fetch(getBaseUrl() + "doable/getComplianceDetail?doableId=" + props.doableId, {
            ...
            .then((data) => {
                ...
                console.log(data);
                setMessages(data);
                setCompletedBy(data[data.length - 1].receiver_first_name + " " + data[data.length - 1].receiver_last_name);
                // setAssignedBy(data.assignedBy);
                // setAssignedTo(data.assignedTo);
        ...
        fetch(getBaseUrl() + "meeting/getUsers", {
            ...
            .then((data) => {
                ...
                data.map((item) => {
                    ...
                    val.push({ label: item.fields.first_name + " " + item.fields.last_name, key: item.pk })
                })
                setparticipants(val);
                setLoadingMember(0);
            })
            ...
            {loading ?
                ...
                <div style={{ boxShadow: '0px 3px 6px 4px #bbb', margin: '10px 0px' }}>
                  ...
                  padding: '10px'
                                    ...
                                    return (
                                        <tr>
                                            ...
                                            <th className='tb' style={{width:'20%'}}>
                                                ...
                                                <Col md='6' style={{marginTop:'10px', marginLeft:'5px'}}><span >{item.relatedDocumentLink.length} Documents</span> </Col>
                                                <Col md='5'>
                                                <Dropdown>
                                                    ...
                                                    {item.relatedDocumentLink.map((i, q)=>{
                                                        ...
                                                        return (<Dropdown.Item target='_blank' href={i}> {s}</Dropdown.Item>)
                                ...
                                {props.completed ?
                                    <tr style={{ background: 'darksalmon', color: 'white', fontWeight: '600' }}>
                                        <th className='tb'></th>
                        ...
                        {!props.completed
                            ?
                            <Row >
                                <Button size='sm' style={{ width: '200px', margin: '10px 20px', background: '#39CCCC', border: 'none' }} onClick={(e) => setForwardMessage(1)}>
                                    Forward Message
                                </Button>
                                <Button size='sm' style={{ width: '200px', margin: '10px 20px', background: '#39CCCC', border: 'none' }} onClick={concludeCompliance}>
                                    Conclude Compliance
                                </Button>
                            </Row>
                            :
                        ...
                        <Row style={{ padding: '15px' }}>
                            <div>
                                ...
                                <Select
                                    name="reciever"
                                    type='text'
                                    placeholder={loadingMember ? "Member Loading..." : "Assign To"}
                                ...
                                />
                                <p>Message</p>
                                <Form.Control as='textarea' style={{ height: '100px', borderColor: 'grey' }} value={subject} placeholder="Subject/Message" onChange={changeSubject} />
                                <BrowseFile />
                                <div
